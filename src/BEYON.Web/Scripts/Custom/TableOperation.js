var m_pagerow = 0;
//初始化Table
function InitTable(tb, actionUrl, dbQueryParams, tbColumns,toolbar) {
    $(tb).bootstrapTable({
...
function MultDelete(table, actionUrl) {
    ...
    for (var i = 0; i < arrselections.length; i++) {
        //alert(arrselections[i].AuditStatus);
        if (arrselections[i].AuditStatus == "审核中" || arrselections[i].AuditStatus == "通过") {
            temp++;  
        }
    }
    if (temp == 0) {
        ...
        WinMsg.confirm({ message: "您选择的数据中有"+temp+"条数据已经提交或审核通过，将不会被删除，确认要删除其他选择的数据吗？" }).on(function (e) {
            if (!e) {
                return;
            }
            $.ajax({
