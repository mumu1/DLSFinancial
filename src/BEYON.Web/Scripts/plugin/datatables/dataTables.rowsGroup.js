...
RowsGroup.prototype = {
	...
	_mergeColumn: function(iStartRow, iFinishRow, columnsIndexes)
	{
		...
		var staticsColumn = this.table.column(currentColumn + 1, ShowedDataSelectorModifier);
		currentColumn = this.table.column(currentColumn, ShowedDataSelectorModifier)
		
		var columnNodes = currentColumn.nodes()
		var columnValues = currentColumn.data()
		
		var staticsColumnNodes = staticsColumn.nodes();
		var staticsColumnValues = staticsColumn.data()
		...
		for (iRow = iStartRow + 1; iRow <= iFinishRow; ++iRow) {
			...
			if (columnValues[iRow] === columnValues[newSequenceRow]) {
				...
				$(staticsColumnNodes[iRow]).hide();
				staticsColumnValues[newSequenceRow] += staticsColumnValues[iRow];
				$(staticsColumnNodes[newSequenceRow]).html(staticsColumnValues[newSequenceRow])
			} else {
				$(columnNodes[newSequenceRow]).show()
				$(columnNodes[newSequenceRow]).attr('rowspan', (iRow-1) - newSequenceRow + 1)
				
				$(staticsColumnNodes[newSequenceRow]).show()
				$(staticsColumnNodes[newSequenceRow]).attr('rowspan', (iRow-1) - newSequenceRow + 1)
