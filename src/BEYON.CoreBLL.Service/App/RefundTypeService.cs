...
namespace BEYON.CoreBLL.Service.App
{
    public class RefundTypeService : CoreServiceBase, IRefundTypeService
    {
        ...
        public OperationResult Insert(RefundTypeVM model)
        {
            try
            {
                RefundType refund = _RefundTypeRepository.Entities.FirstOrDefault(c => c.RefundTypeName == model.RefundTypeName.Trim());
                if (refund != null)
                {
        ...
        public OperationResult Delete(List<string> refundTypeName)
        {
            try
            {
                if (refundTypeName != null)
                {
                    int count = _RefundTypeRepository.Delete(_RefundTypeRepository.Entities.Where(c =>refundTypeName.Contains(c.RefundTypeName)));
                    if (count > 0)
                    {
