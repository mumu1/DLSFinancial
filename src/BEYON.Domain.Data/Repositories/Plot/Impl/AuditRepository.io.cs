...
namespace BEYON.Domain.Data.Repositories.Plot.Impl
{
    ...
    {
        public IList<Audit> Export(IList<String> umrids)
        {
            var q = from p in Context.Audits
                    where umrids.Contains(p.UmrID)
                    select p;
            return q.ToList();
        }

        public void InsertOrUpdate(IQueryable<Audit> umrcovers)
        {
            if (umrcovers == null)
                return;
            ...
            {
                Audit oldCover = Context.Audits.Where(t => t.UmrID == item.UmrID).FirstOrDefault();
                if (oldCover == null)
                {
                    this.Insert(item);
                }
