...
namespace BEYON.Domain.Data.Repositories.Plot.Impl
{
    public partial class ProtectUnitAuditRepository
    {
        ...
        private NotifyItem GetPassOrNotAudit(string columName, String userID, string auditStatus)
        {
            ...
            {
                case "OperatorID":
                    count = (from p in Context.ProtectUnitAudits.Where(w=>w.OperatorID == userID && w.AuditStatus == auditStatus) select p).Count();
                    break;
                case "LeaderAuditorID":
                    count = (from p in Context.ProtectUnitAudits.Where(w => w.LeaderAuditorID == userID && w.AuditStatus == auditStatus) select p).Count();
                    break;
                case "InstituteAuditorID":
                    count = (from p in Context.ProtectUnitAudits.Where(w => w.InstituteAuditorID == userID && w.AuditStatus == auditStatus) select p).Count();
                    break;
                case "BureauAuditorID":
                    count = (from p in Context.ProtectUnitAudits.Where(w => w.BureauAuditorID == userID && w.AuditStatus == auditStatus) select p).Count();
                    break;
            }
        ...
        private NotifyItem GetNeedAudit(string columnName)
        {
            ...
            {
                case "LeaderAuditorID":
                    count = (from p in Context.ProtectUnitAudits.Where(w => w.AuditStatus == "提交" && String.IsNullOrEmpty(w.LeaderAuditorID)) select p).Count();
                    break;
                case "InstituteAuditorID":
                    count = (from p in Context.ProtectUnitAudits.Where(w => w.LeaderAuditStatus == "通过" && String.IsNullOrEmpty(w.InstituteAuditorID)) select p).Count();
                    break;
                case "BureauAuditorID":
                    count = (from p in Context.ProtectUnitAudits.Where(w => w.InstituteAuditStatus == "通过" && String.IsNullOrEmpty(w.BureauAuditorID)) select p).Count();
                    break;
            }
